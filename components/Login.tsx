  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (errCode === 'auth/invalid-credential' || errCode === 'auth/wrong-password' || errCode === 'auth/user-not-found') {
            if (originalId === 'admin') {
                setError(`登入失敗。若是首次使用，請務必至 Firebase Console 手動建立帳號：${email} (密碼任意)`);
            } else {
                setError(`帳號或密碼錯誤 (嘗試登入: ${email})`);
            }
    ...
    <div className="min-h-full flex items-center justify-center p-4 bg-gray-100 font-bold overflow-y-auto relative">
      <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden flex flex-col border border-gray-100 relative p-8 md:p-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
          <input type="text" required value={username} onChange={e=>setUsername(e.target.value)} className="w-full p-4 bg-white border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-brand-500 font-black transition-all" placeholder="帳號 (Email 或 ID)" />
